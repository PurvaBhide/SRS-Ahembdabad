...
// ⭐ NEW: Helper function to format a single budget amount (e.g., 2000000 -> ₹20 Lakh)
...
function updateProjectDetails(project) {
  ...
  // ⭐ MODIFIED: Format the budget using formatSingleAmount
  const formattedBudget = formatSingleAmount(project.projectBudget);
  ...
  const descriptionHTML = `
    <p><strong>Project Name:</strong> ${project.projectName || 'Not specified'}</p>
    <p><strong>Estimated Cost:</strong> ${formattedBudget}</p> <p><strong>Short Description:</strong> ${project.projectShortDescription || 'No description available'}</p>
    ${project.projectLongDescription ? `<p><strong>Detailed Description:</strong> ${project.projectLongDescription}</p>` : ''}
  `;
  document.querySelector('.project-description').innerHTML = descriptionHTML;
...
//   gallery tab content
  ...
  // Update funding target
  document.querySelector('.raised-amount').textContent = `${formattedBudget}`; // ⭐ USED HERE
  document.querySelector('#customAmount').value = parseInt(project.projectBudget || 0);
}
...
// Helper function to format currency (Kept for completeness, though formatSingleAmount is used now)
function formatCurrency(amount) {
  return new Intl.NumberFormat('en-IN', {
    style: 'currency',
    currency: 'INR',
