...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ⭐ NEW: Helper function to format a single budget amount for display
  ...
  function createProjectCard(project) {
    ...
    const budget = formatSingleAmount(project.projectBudget);
...
">${escapeHtml(
  ...
  function displayNoProjectsWithDetails(categoryId, budgetRange) {
    ...
    // ⭐ MODIFIED: Use the range formatter for display
...
// ⭐ UPDATED: Helper function to format budget range display (now uses formatSingleAmount internally)
function formatBudgetRange(rangeString) {
  if (!rangeString || rangeString === "all") {
    return "All Budgets";
  }
...
// ⭐ NEW: Helper function to format a single budget amount (extracted from previous version)
function formatSingleAmount(amount) {
    const budget = parseInt(amount) || 0;
