...
//     currentFilters.search = query;   // ✅ update filter state
...
//           const desc = project.projectShortDescription?.toLowerCase() || '';
//           return name.includes(query.toLowerCase()) || desc.includes(query.toLowerCase());
//         });

//         if (!filtered.length) {
//           projectList.innerHTML = '<div class="no-results">No projects match your search</div>';
//           return;
//         }
//         projectList.innerHTML = filtered.map(renderProjectCard).join('');
//       } catch (err) {
//         console.error('Search failed:', err);
//         projectList.innerHTML = '<div class="error">Unable to search projects</div>';
//       }
//     }

//     // Load projects based on current filters and pagination
//     function loadProjectsWithFilters() {
//         const {category, budget, search } = currentFilters;
        
//         console.log('Loading projects with filters:', { category, budget, currentPage, pageSize });
//           if (search && search.length > 0) {
//         console.log('Searching projects with query:', search);
//         loadProjectsBySearch(search, category, budget);
//         return;
//     }
...
// function loadProjectsBySearch(query) {
//   projectList.innerHTML = '<div class="loading">Searching projects...</div>';

//   fetch(`https://mumbailocal.org:8087/projects/search?keyword=${encodeURIComponent(query)}`)
//     .then(res => res.json())
//     .then(response => {
//       const projects = filterActiveProjects(response.data || []); // ✅ FIX
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function formatBudgetRange(rangeString) {
        ...
        // Use the new single amount formatter for min and max
...
function loadProjectsBySearch(query) {
  ...
  fetch(`https://mumbailocal.org:8087/projects/search?keyword=${encodeURIComponent(query)}`)
    ...
    function createProjectCard(project) {
        const budget = project.projectBudget ;
        // ⭐ MODIFIED: Format the budget amount for display
        const formattedBudget = formatSingleAmount(budget);
        ...
        return `
            <div class="card" data-project-id="${projectId}">
                ...
                <div class="card-content">
                    <div class="tag">${escapeHtml(category)}</div>
                    <div class="cost">
                        Funding Required: <span class="cost-amount">${formattedBudget} </span>
                    </div>
                    <h3 class="title">${escapeHtml(title)}</h3>
                    <p class="description">${escapeHtml(description.substring(0, 120))}${description.length > 120 ? '...' : ''}</p>
                    ${raisedAmount > 0 ? `
                        <div class="donate-progress">
                            <div class="progress-info">
                                <span class="raised-amount">₹${Number(raisedAmount).toLocaleString('en-IN')} raised</span>
                                <span class="target-amount">of ${formattedBudget}</span>
                            </div>
                            <div class="progress-bar">
                                <div class="progress-fill" style="width: ${Math.min(progressPercentage, 100)}%"></div>
                            </div>
